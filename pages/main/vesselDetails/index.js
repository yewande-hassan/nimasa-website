    ...
    <>
      ...
      <main className="row container-fluid">
        <section className={`col-5 ${styles.section1}`}>
          <div
            className={`d-flex flex-row mt-2 justify-content-between ${styles.contain}`}
          >
            <div>
          ...
          {details.map((rows, index) => (
            <Details key={index} column={rows.column} />
          ))}
        </section>
        <section className={`col-7 `}>
          <div>
            ...
            <div
              className={`d-flex justify-content-between py-2 ${styles.head} ${styles.transac} ${styles.row}`}
            >
            ...
            {transaction.map((rows, index) => (
              <Transaction key={index} column={rows.column} />
            ))}
          </div>
        </section>
      </main>
